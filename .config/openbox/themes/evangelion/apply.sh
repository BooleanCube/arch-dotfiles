...
apply_rofi() {
	...
	sed -i -e "s/STYLE=.*/STYLE=\"$THEME\"/g" \
		${PATH_OBOX}/scripts/askpass \
		${PATH_OBOX}/scripts/launcher \
		${PATH_OBOX}/scripts/music \
		${PATH_OBOX}/scripts/powermenu \
		${PATH_OBOX}/scripts/runner \
		${PATH_OBOX}/scripts/screenshot
